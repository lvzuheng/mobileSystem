...
function init_data(start, end) {
	...
	$.ajax({
		url: uri + "searchOperatorList",
		type: "get",
		data: { "request": JSON.stringify(jsondata) },
		async: true,
		success: function(result) {
			$('#wrapper_table').html("");
			init_OperatorList(JSON.parse(result));
		...
		success: function(result) {
			setTimeout(function() {
				if(JSON.parse(result) == "" || JSON.parse(result) == null) {
					...
					$("#div_scroll_pulldown")[0].style.display = "none";
					myScroll.refresh();
					var info_d = info_dialog("没有更多用户了");
					info_d.show();
					setTimeout(function() {
						info_d.close();
					}, 2000);
...
function init_OperatorList(list) {
	...
	if(table.children().length <= 0) {
		...
		i_tr.appendChild(createElemet("td", "角色类型"));
	...
	for(var i = 0; i < list.length; i++) {
		...
		tr.appendChild(createElemet("td", list[i].organizationName));
		tr.appendChild(createElemet("td", list[i].charactor));
